...
class APISettingsMigration {
  ...
  async init() {
    try {
      ...
      if (window.SecureAPIStorage) {
        ...
        this.logger.debug('✅ API Settings Migration System v2.0 initialized with enhanced features');
        return true;
      } else {
        this.logger.warn('⚠️ SecureAPIStorage not available after waiting - migration disabled');
        return false;
      }
    } catch (error) {
      this.logger.error('❌ Failed to initialize API migration:', error);
      return false;
    }
  }
  ...
  getExistingData() {
    try {
      ...
      this.logger.error('❌ Failed to read existing API data:', error);
      return {};
    }
  }
  ...
  async migrateAPISettings() {
    try {
      ...
      if (saveResult.success) {
        ...
        this.logger.debug('✅ API settings migrated successfully');
        return {
          success: true,
          migrated: true,
      ...
      this.logger.error('❌ API migration failed:', error);
      return {
        success: false,
        migrated: false,
        error: error.message
  ...
  cleanupOldStorage() {
    try {
      this.oldKeys.forEach(key => {
        ...
        this.logger.debug(`🧹 Removed old localStorage key: ${key}`);
      });
    } catch (error) {
      this.logger.error('❌ Failed to cleanup old storage:', error);
    }
  }
  ...
  showBannerNotification(resolve, existingData, hasApiKey, hasModel, options = {}) {
    ...
    this.logger.debug('API migration check completed silently');
  ...
  setNotificationStyle(style) {
    if (['banner', 'corner', 'sidebar', 'modal'].includes(style)) {
      ...
      this.logger.debug(`🔧 API migration notification style set to: ${style}`);
      return true;
    }
    this.logger.warn(`⚠️ Invalid notification style: ${style}. Use: banner, corner, sidebar, or modal`);
    return false;
  }
  ...
  saveNotificationPreferences(preferences) {
    try {
      ...
      this.logger.debug('✅ Notification preferences saved:', updatedPrefs);
      return true;
    } catch (error) {
  ...
  async migrateAPISettingsWithRetry(maxRetries = 3) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        this.logger.debug(`🔄 Migration attempt ${attempt}/${maxRetries}`);
        const result = await this.migrateAPISettings();
        
        if (result.success) {
          this.logger.debug('✅ Migration successful on attempt', attempt);
          return result;
        } else {
        ...
        lastError = error;
        this.logger.warn(`⚠️ Migration attempt ${attempt} failed:`, error.message);
    ...
    this.logger.error('❌ Migration failed after', maxRetries, 'attempts:', lastError);
    return {
      success: false,
  ...
  cleanup() {
    try {
      ...
      this.logger.debug('🧹 Notification system cleaned up');
    } catch (error) {
  ...
  showPreferencesPanel() {
    ...
    panel.querySelector('#prefs-save').onclick = () => {
      ...
      if (this.saveNotificationPreferences(newPrefs)) {
        panel.classList.remove('visible');
        setTimeout(() => panel.remove(), 300);
        this.logger.debug('✅ Preferences saved successfully');
      }
    };
    ...
    panel.querySelector('#prefs-reset').onclick = () => {
      ...
      panel.classList.remove('visible');
      setTimeout(() => panel.remove(), 300);
      this.logger.debug('🔄 Preferences reset to defaults');
    };
  ...
  initKeyboardShortcuts() {
    document.addEventListener('keydown', (event) => {
      ...
      if (event.ctrlKey && event.shiftKey && event.key === 'M') {
        ...
        this.logger.debug('🔧 Keyboard shortcut: Show migration notification');
      ...
      if (event.ctrlKey && event.shiftKey && event.key === 'P') {
        event.preventDefault();
        this.logger.debug('🔧 Keyboard shortcut: Show preferences panel');
      ...
      if (event.ctrlKey && event.shiftKey && event.key === 'T') {
        event.preventDefault();
        this.logger.debug('🔧 Keyboard shortcut: Test all notification types');
      ...
      if (event.ctrlKey && event.shiftKey && event.key === 'D') {
        event.preventDefault();
        this.logger.debug('🔧 Keyboard shortcut: Show notification demo');
      ...
      if (event.ctrlKey && event.shiftKey && event.key === 'H') {
        event.preventDefault();
        this.logger.debug('🔧 Keyboard shortcut: Show help dialog');
        this.showHelpDialog();
      }
    });
    
    this.logger.debug('⌨️ Keyboard shortcuts initialized:');
    this.logger.debug('  Ctrl+Shift+M: Show test migration notification');
    this.logger.debug('  Ctrl+Shift+P: Show preferences panel');
    this.logger.debug('  Ctrl+Shift+T: Test all notification types');
    this.logger.debug('  Ctrl+Shift+D: Show notification demo');
    this.logger.debug('  Ctrl+Shift+H: Show help dialog');
  }
  ...
  showNotificationDemo() {
    this.logger.debug('🎭 Starting notification system demo...');
    ...
    this.logger.debug('✅ Notification demo started - watch for the sequence of notifications!');
  }
  ...
  initializePreferences() {
    ...
    this.logger.debug('🎨 Preferences initialized:', prefs);
  }
  ...
  async initAutoMigration(notificationStyle = null) {
    try {
      ...
      if (!initialized) {
        this.logger.warn('⚠️ Secure storage not available - migration disabled');
        return false;
      }
      ...
      if (this.shouldShowReminder()) {
        ...
        const debouncedShow = this.debounce(() => {
          ...
          this.showMigrationPrompt(style, prefs).then(result => {
            this.logger.debug('🔒 API migration prompt result:', result);
            ...
            this.logger.error('❌ Migration prompt failed:', error);
          });
        }, 500);
      ...
      this.logger.error('❌ Auto migration initialization failed:', error);
      return false;
    }
  }
}
...
this.logger.debug('✅ API Settings Migration Helper loaded');
