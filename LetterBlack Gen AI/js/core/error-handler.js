...
class ErrorHandler {
  ...
  logError(errorInfo) {
    ...
    switch (errorInfo.severity) {
      case 'critical':
        this.logger.error('🚨 CRITICAL:', logMessage, errorInfo);
        break;
      case 'high':
        this.logger.error('❌ ERROR:', logMessage, errorInfo);
        break;
      case 'medium':
        this.logger.warn('⚠️ WARNING:', logMessage, errorInfo);
        break;
      case 'low':
        console.info('ℹ️ INFO:', logMessage, errorInfo);
        break;
      default:
        this.logger.debug('📝 LOG:', logMessage, errorInfo);
    }
  }
