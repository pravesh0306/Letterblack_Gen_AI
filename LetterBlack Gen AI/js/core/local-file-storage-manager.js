...
class LocalFileStorageManager {
    ...
    async init() {
        ...
        this.logger.debug('✅ Local File Storage Manager initialized');
    }
    ...
    async loadStoredFiles() {
        try {
            ...
            this.logger.warn('Failed to load stored files:', error);
        }
    }
    ...
    saveFiles() {
        try {
            ...
            this.logger.error('Failed to save files:', error);
    ...
    async processUploadQueue() {
        ...
        try {
            ...
            this.logger.error('Error processing upload queue:', error);
        } finally {
            this.processing = false;
    ...
    cleanupOldFiles() {
        ...
        if (cleaned > 0) {
            this.logger.debug(`Cleaned up ${cleaned} old temporary files`);
        }
    }
    ...
    notifyObservers(event, data) {
        this.observers.forEach(callback => {
            try {
                ...
                this.logger.error('Error in file storage observer:', error);
            }
        });
    }
    ...
    setupEventListeners() {
        ...
        this.subscribe((event, data) => {
            if (event === 'file_added' && this.getTotalStorageSize() > this.maxStorageSize * 0.9) {
                this.logger.warn('Storage space is running low');
            }
        });
    }
    ...
    async exportFiles(fileIds = null) {
        ...
        this.logger.debug('Exporting files:', filesToExport.map(f => f.name));
