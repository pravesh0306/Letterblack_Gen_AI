...
class MemoryManager {
  ...
  checkMemoryUsage() {
    ...
    this.logger.debug(`Memory Usage: ${(usedBytes / 1024 / 1024).toFixed(2)}MB / ${(limitBytes / 1024 / 1024).toFixed(2)}MB`);

    // Warn if memory usage is high
    if (usedBytes > this.memoryThreshold) {
      this.logger.warn('High memory usage detected, performing cleanup');
      this.performCleanup();
    }
  ...
  performCleanup() {
    ...
    try {
      ...
      if (cleaned > 0) {
        this.logger.debug(`Cleaned up ${cleaned} memory references`);
      }
  ...
  cleanupDOMReferences() {
    ...
    if (removed > 0) {
      this.logger.debug(`Removed ${removed} detached DOM nodes`);
    }
  }
  ...
  monitorObject(obj, name) {
    ...
    setTimeout(() => {
      if (!monitor.weakRef.deref()) {
        this.logger.debug(`Object ${name} was properly garbage collected`);
      } else {
        this.logger.warn(`Object ${name} may have memory leak - still alive after timeout`);
      }
   ...
   */
  cleanup() {
    this.logger.debug('Performing complete memory cleanup...');
