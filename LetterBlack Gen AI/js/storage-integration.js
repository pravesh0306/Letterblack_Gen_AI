...
class ChatStorageManager {
  ...
  async initSecureStorage() {
    try {
      ...
      this.logger.warn('SecureAPIStorage not available:', error);
    }
  }
  ...
  async getSecureSetting(key, defaultValue) {
    try {
      if (this.secureStorage) {
        ...
        this.logger.warn(`🚫 Using insecure localStorage for ${key} - SecureAPIStorage unavailable`);
        return localStorage.getItem(key) || defaultValue;
      }
    } catch (error) {
      this.logger.error(`Failed to get secure setting ${key}:`, error);
      return defaultValue;
    }
  }
  ...
  async initialize() {
    try {
      this.logger.debug("🚀 Initializing Chat Storage Manager...");
      ...
      if (window.chatMigrationHelper) {
        ...
        this.logger.debug("🔄 Migration result:", migrationResult);
      }
      ...
      this.logger.debug("✅ Chat Storage Manager initialized");
      this.logger.debug(`📁 Active conversation: ${this.activeConversationId}`);
      
      return true;
    } catch (error) {
      this.logger.error("❌ Failed to initialize Chat Storage Manager:", error);
  ...
  wireUIElements() {
    ...
    this.logger.debug("🔗 UI elements wired up");
  }
  ...
  getAEContext() {
    try {
      ...
      this.logger.warn("Could not get AE context:", error);
    }
  ...
  async loadMessagesIntoUI() {
    ...
    try {
      ...
      this.scrollToBottom();
      this.logger.debug(`📖 Loaded ${conversation.messages.length} messages into UI`);
  ...
  async syncFromUI() {
    ...
    try {
      ...
      this.logger.debug("🔄 UI sync detected changes, but storage is authoritative");
      
    } catch (error) {
      this.logger.warn("⚠️ UI sync failed:", error);
    }
  }
  ...
  showToast(message, type = 'info') {
    ...
    // Fallback to console
    this.logger.debug(`🍞 Toast (${type}): ${message}`);
  }
   ...
   */
  cleanup() {
    this.logger.debug("🧹 Cleaning up ChatStorageManager...");
    ...
    this.logger.debug("✅ ChatStorageManager cleanup complete");
  }
}
