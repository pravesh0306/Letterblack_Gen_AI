    ...
    class ChatMemory {
        ...
        initializeConversationFolder() {
            try {
                if (window.CSInterface) {
                    ...
                    `, (result) => {
                        this.logger.debug('💬 Chat folder initialized:', this.conversationFolderPath);
                    });
                }
            } catch (error) {
                this.logger.warn('Failed to initialize conversation folder:', error);
        ...
        addMessage(type, message) {
            try {
                ...
                this.logger.error('ChatMemory.addMessage error:', error);
            }
        }
            ...
            try {
                ...
                `, (result) => {
                    if (result !== 'Message saved to file') {
                        this.logger.warn('Failed to save message to file:', result);
                    }
                });
                
            } catch (error) {
                this.logger.error('Failed to save conversation to file:', error);
            }
        }
        ...
        getHistory() {
            try {
                ...
                this.logger.error('ChatMemory.getHistory error:', error);
                return [];
            }
        }
        ...
        getRecentHistory(limit = null) {
            try {
                ...
                this.logger.error('ChatMemory.getRecentHistory error:', error);
                return [];
            }
        }
        ...
        getContextString(limit = 6) {
            try {
                ...
                this.logger.error('ChatMemory.getContextString error:', error);
                return '';
            }
        }
        ...
        getTopicHistory(topic, maxResults = 10) {
            try {
                ...
                this.logger.error('ChatMemory.getTopicHistory error:', error);
                return [];
            }
        }
        ...
        trimHistory(keepRecentCount = 50) {
            try {
                ...
                this.logger.error('ChatMemory.trimHistory error:', error);
            }
        }
        ...
        startNewSession() {
            this.currentSessionId = this.generateSessionId();
            this.logger.debug('📝 Started new chat session:', this.currentSessionId);
        }
            ...
            return new Promise((resolve) => {
                ...
                `, (result) => {
                    try {
                        ...
                        this.logger.error('Error parsing sessions:', error);
                        resolve([]);
                    }
                });
            });
        ...
        exportConversationHistory() {
            return new Promise(async (resolve) => {
                try {
                    ...
                    this.logger.error('Error exporting conversation history:', error);
                    resolve(null);
                }
            });
        }
        ...
        clearHistory() {
            try {
                ...
                this.logger.error('ChatMemory.clearHistory error:', error);
            }
        }
    }
