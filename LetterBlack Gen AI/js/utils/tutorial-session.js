    ...
    class TutorialSessionManager {
        ...
        _load(){
            try {
                ...
                this.logger.warn('TutorialSession load failed:', e.message);
                return {};
            }
        }

        _persist(){
            try { localStorage.setItem(STORAGE_KEY, JSON.stringify(this.sessions)); } catch(e){ this.logger.warn('Persist tutorial sessions failed:', e.message); }
        }
