...
class BrowserVideoTranscriber {
    constructor() {
        ...
        this.logger.debug('🎬 Browser Video Transcriber initialized');
    }
    ...
    async transcribeVideo(videoUrl, videoId) {
        ...
        if (this.transcriptionCache.has(videoId)) {
            this.logger.debug('📋 Using cached transcript');
            return this.transcriptionCache.get(videoId);
        }

        try {
            this.logger.debug('🎬 Attempting to analyze YouTube video...');
            ...
            this.logger.debug('❌ Video analysis not available:', error.message);
    ...
    clearCache() {
        ...
        this.logger.debug('🗑️ Video transcription cache cleared');
    }
