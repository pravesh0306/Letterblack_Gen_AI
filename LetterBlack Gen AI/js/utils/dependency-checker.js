...
class DependencyChecker {
    ...
    async checkAllDependencies() {
        this.logger.debug('🔍 Checking system dependencies...');
        ...
        for (const [name, dep] of Object.entries(this.dependencies)) {
            try {
                ...
                this.logger.error(`Error checking ${name}:`, error);
                results.details[name] = {
                    available: false,
                    required: dep.required,
                    description: dep.description,
    ...
    async showDependencyStatus() {
        ...
        this.logger.debug(`✅ Passed: ${results.passed}/${results.total}`);
        this.logger.debug(`❌ Failed: ${results.failed}/${results.total}`);
        this.logger.debug(`🔧 Required: ${results.required}`);
        this.logger.debug(`⚡ Optional: ${results.optional}`);
        this.logger.debug('─'.repeat(50));
        ...
        if (categories.ok.length > 0) {
            ...
            categories.ok.forEach(dep => {
                this.logger.debug(`  ✓ ${dep.name}: ${dep.description}`);
            });
        }
        ...
        if (categories.error.length > 0) {
            ...
            categories.error.forEach(dep => {
                this.logger.debug(`  ❌ ${dep.name}: ${dep.description}`);
                if (dep.error) {
                    this.logger.debug(`     Error: ${dep.error}`);
                }
            });
        }
    ...
    showUIWarning(count, errors) {
        ...
        this.logger.debug('\n🔧 RECOMMENDED ACTIONS:');
        errors.forEach(dep => {
            this.logger.debug(`  • ${dep.name}: ${this.getRecommendation(dep.name)}`);
        });
    }
    ...
    showUISuccess() {
        ...
        this.logger.debug('🎉 All critical dependencies are available!');
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!window.dependencyChecker) {
        ...
        this.logger.debug('✅ Dependency Checker initialized');
