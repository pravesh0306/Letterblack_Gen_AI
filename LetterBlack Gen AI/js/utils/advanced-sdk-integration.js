...
class AdvancedSDKIntegration {
    ...
    async init() {
        if (!this.csInterface) {
            this.logger.error('❌ CSInterface not available - Advanced SDK features disabled');
            return;
        }
        ...
        try {
            ...
            this.logger.error('Failed to initialize Advanced SDK:', error);
        }
    }
    ...
    cleanup() {
        if (this.pollInterval) {
            ...
            this.logger.debug('🧹 Advanced SDK cleanup completed');
        }
    ...
    setupAdvancedEventListeners() {
        ...
        this.csInterface.addEventListener('com.adobe.csxs.events.ApplicationActivate', (event) => {
            this.logger.debug('🎬 After Effects activated - refreshing AI context');
            ...
            this.csInterface.evalScript(advancedScript, (result) => {
                try {
                    ...
                    if (analysis.status === 'error') {
                        this.logger.error('Advanced layer analysis script failed:', analysis.message);
                        resolve(null);
                    } else {
                        resolve(analysis);
                    }
                } catch (error) {
                    this.logger.error('Failed to parse advanced layer analysis result:', error, result);
                    resolve(null);
                }
            });
        });
            ...
            this.csInterface.evalScript(smartEffectScript, (result) => {
                try {
                    ...
                    if (response.status === 'error') {
                        this.logger.error('Smart effect script returned an error:', response.message);
                        resolve(false);
                    } else {
                        resolve(response.status === 'success');
                    }
                } catch (error) {
                    this.logger.error('Failed to parse smart effect result:', error, result);
                    resolve(false);
                }
            });
        });
        ...
        return new Promise((resolve) => {
            ...
            this.csInterface.evalScript(expressionScript, (result) => {
                try {
                    const response = JSON.parse(result);
                     if (response.status === 'error') {
                        this.logger.error('Intelligent expression script failed:', response.message);
                    }
                    resolve(response);
                } catch (error) {
                    this.logger.error('Failed to parse intelligent expression result:', error, result);
        ...
        return new Promise((resolve) => {
            ...
            this.csInterface.evalScript(creationScript, (result) => {
                try {
                    const response = JSON.parse(result);
                    if (response.status === 'error') {
                        this.logger.error('Intelligent composition script failed:', response.message);
                        resolve(null);
                    } else {
                        resolve(response);
                    }
                } catch (error) {
                    this.logger.error('Failed to parse intelligent composition result:', error, result);
                    resolve(null);
                }
            });
        });
            ...
            this.csInterface.evalScript(metricsScript, (result) => {
                try {
                    ...
                    if (response.status === 'success') {
                        ...
                        this.logger.error('Performance metrics script failed:', response.message);
                        resolve(null);
                    }
                } catch (error) {
                    this.logger.error('Failed to parse performance metrics result:', error, result);
                    resolve(null);
                }
            });
        });
