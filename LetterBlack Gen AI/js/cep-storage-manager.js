...
class CEPSettingsManager {
    ...
    async saveSettings(settings) {
        ...
        try {
            ...
            this.logger.debug('✅ Settings saved successfully (localStorage + file backup)');
            return { success: true, method: 'localStorage+file', timestamp };
        } catch (error) {
            this.logger.warn('❌ Storage failed:', error);
    ...
    async loadSettings() {
        try {
            ...
            if (stored) {
                ...
                this.logger.debug('📄 Settings loaded from primary storage');
            ...
            if (Object.keys(fallbackSettings).length > 0) {
                this.logger.debug('📄 Settings loaded from legacy format');
            ...
            if (this.fallbackEnabled) {
                ...
                if (fileSettings && Object.keys(fileSettings).length > 0) {
                    this.logger.debug('📄 Settings loaded from file backup');
                    return { ...this.defaultSettings, ...fileSettings };
                }
            }
            
            this.logger.debug('📄 Using default settings (no saved data found)');
            return this.defaultSettings;
        } catch (error) {
            this.logger.warn('⚠️ Loading failed, using defaults:', error);
            return this.defaultSettings;
        }
    }
    ...
    async saveToFile(settings) {
        try {
            ...
            if (typeof window !== 'undefined' && window.cep && window.cep.fs) {
                ...
                this.logger.debug('💾 File backup saved');
            }
        } catch (error) {
            this.logger.warn('File backup failed (not critical):', error);
        }
    }
    ...
    async loadFromFile() {
        try {
            ...
            if (typeof window !== 'undefined' && window.cep && window.cep.fs) {
                ...
                this.logger.debug('📂 Loaded from file backup');
                return {};
            }
        } catch (error) {
            this.logger.warn('File load failed (not critical):', error);
        }
        return {};
    }
    ...
    clearSettings() {
        try {
            ...
            this.logger.debug('🗑️ All settings cleared');
            return { success: true };
        } catch (error) {
            this.logger.error('❌ Failed to clear settings:', error);
            return { success: false, error: error.message };
        }
    }
    ...
    runHealthCheck() {
        ...
        try {
            ...
            this.logger.debug('🔍 Storage health check completed:', results);
            return results;
        } catch (error) {
