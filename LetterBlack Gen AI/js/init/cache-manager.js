...
function deleteCacheAndReload() {
    this.logger.debug('%c🗑️ DELETING ALL CACHES & FORCING RELOAD...', 'background: #ff4444; color: white; font-weight: bold; padding: 8px; border-radius: 4px;');
    ...
    try {
        ...
        if ('caches' in window) {
            caches.keys().then(names => {
                names.forEach(name => {
                    caches.delete(name);
                    this.logger.debug(`Cache deleted: ${name}`);
                });
            });
        }
        ...
        if (window.CSInterface) {
            try {
                ...
                this.logger.debug('CEP not available for cache clearing');
            }
        }
        ...
        this.logger.error('Cache deletion error:', error);
...
(function safeInitialization() {
    this.logger.debug('🚀 Safe initialization starting...');
    ...
    if (missingGlobals.length > 0) {
        this.logger.warn('⚠️ Missing required globals:', missingGlobals);
    ...
    function initializeModuleSafely(name, fn) {
        try {
            ...
            this.logger.warn(`⚠️ ${name} initialization failed:`, error.message);
        }
    }
    ...
    if (typeof window.SecureAPIStorage !== 'undefined' && !window.SecureAPIStorage.placeholder) {
        this.logger.debug('✅ SecureAPIStorage available');
    }
    
    this.logger.debug('✅ Safe initialization complete');
})();
