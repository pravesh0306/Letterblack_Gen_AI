...
(function(){
    ...
    function checkAIModuleAvailability() {
        ...
        const checkInterval = setInterval(() => {
            ...
            if (window.aiModule) {
                this.logger.debug('✅ AI Module is available');
            ...
            if (checks >= maxChecks) {
                this.logger.warn('⚠️ AI Module not available after', maxChecks, 'checks');
                updateMascotMessage('AI Module loading...');
                clearInterval(checkInterval);
                
                // Try to trigger initialization manually
                if (window.AIModule && !window.aiModule) {
                    this.logger.debug('🔄 Attempting to initialize AI Module manually...');
                    try {
                        ...
                        this.logger.error('❌ Manual AI Module initialization failed:', error);
                        updateMascotMessage('AI Module Error');
                        triggerMascotAnimation('mascot-error');
                    }
                }
    ...
    function cleanup() {
        if (window._uiEnhancementsCleanup) {
            ...
            this.logger.debug('🧹 UI Enhancements cleanup completed');
        }
    }
