  ...
  [base, logsDir].forEach(p => {
    if (!fs.existsSync(p)) {
      try {
        ...
        this.logger.debug(`✅ Created directory: ${p}`);
      } catch (error) {
        this.logger.error(`❌ Failed to create directory ${p}:`, error.message);
        throw error;
      }
    }
  });
...
function readJSON(file) {
  try {
    ...
    this.logger.warn(`⚠️ Failed to read JSON from ${file}:`, error.message);
    return null;
  }
}
...
function atomicWrite(file, content) {
  ...
  try {
    ...
    this.logger.debug(`✅ Atomic write successful: ${file}`);
  } catch (error) {
    // Clean up temp file if it exists
    if (fs.existsSync(tmp)) {
      try { fs.unlinkSync(tmp); } catch {}
    }
    this.logger.error(`❌ Atomic write failed for ${file}:`, error.message);
    throw error;
  }
}
...
function loadChat() {
  try {
    ...
    this.logger.error("❌ Failed to load chat data:", error.message);
...
function rotateIfNeeded(maxBytes = 2_000_000) {
  try {
    ...
    this.logger.debug(`🔄 Rotated chat log: ${finalDst}`);
    return true;
  } catch (error) {
...
function createConversation(title = "New Conversation") {
  try {
    ...
    saveChat(data);
    this.logger.debug(`✅ Created conversation: ${title} (${id})`);
    return id;
  } catch (error) {
...
async function appendMessage(conversationId, message) {
  try {
    ...
    this.logger.debug(`✅ Message appended to conversation ${conversationId}`);
    return newMessage;
  } catch (error) {
    this.logger.error("❌ Failed to append message:", error.message);
    throw error;
  }
}
...
function clearAll() {
  try {
    ...
    atomicWrite(active, getDefaultData());
    this.logger.debug("🗑️ Chat history cleared");
    return true;
  } catch (error) {
...
function getConversationList() {
  try {
    ...
    this.logger.error("❌ Failed to get conversation list:", error.message);
    return [];
  }
}
...
function exportToFile(filePath) {
  try {
    ...
    this.logger.debug(`📤 Chat data exported to: ${filePath}`);
    return true;
  } catch (error) {
