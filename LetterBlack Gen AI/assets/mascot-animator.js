...
class MascotAnimator {
    ...
    init() {
        ...
        if (!this.mascotVideo) {
            this.logger.warn('Mascot video element not found');
            return;
        }
        ...
        this.loadManifest().then(() => this.validateAssets()).then(missing => {
            if (missing.length) {
                this.logger.warn('Mascot missing or unverifiable pose assets (fallback to Idle when needed):', missing);
            }
            this.playScenario('idle');
        });
        
        this.logger.debug('🎭 WebM Mascot Animation System initialized');
    }
    ...
    async validateAssets() {
        ...
        await Promise.all(Object.entries(this.scenarios).map(async ([key, data]) => {
            ...
            try {
                ...
                this.logger.warn('Mascot pose not verifiable (will attempt play anyway):', key, e.message);
                missing.push(key);
            }
        }));
    ...
    async loadManifest() {
        try {
            ...
            if (res.ok) {
                ...
                this.logger.debug('Mascot manifest loaded', this.manifest.generated);
            }
        } catch (e) {
    ...
    setupEventListeners() {
        ...
        if (!mascotContainer) {
            this.logger.warn('Mascot container not found');
            return;
        }
        ...
        this.mascotVideo.addEventListener('loadeddata', () => {
            this.logger.debug('✅ WebM mascot video loaded successfully');
        });
        
        // Handle video loading errors
        this.mascotVideo.addEventListener('error', () => {
            this.logger.warn('WebM mascot failed to load, showing fallback');
            this.showFallback();
        });
    ...
    showFallback() {
        ...
        if (fallback) {
            ...
            this.logger.debug('🎭 Switched to fallback GIF animation');
        }
    }
    ...
    handlePlaybackError(attemptedScenario) {
        ...
        if (attemptedScenario === 'idle') {
            this.logger.warn('Idle video failed, switching to GIF fallback');
            this.showFallback();
            return;
        }
        
        // Try to fall back to idle video first
        this.logger.warn(`${attemptedScenario} video failed, trying idle animation`);
        this.playScenario('idle');
    }
    ...
    playScenario(scenario, immediate = false) {
        if (!this.scenarios[scenario]) {
            this.logger.warn(`Unknown mascot scenario: ${scenario}, falling back to idle`);
        ...
        const playVideo = async () => {
            try {
                ...
                this.isPlaying = true;
                this.logger.debug(`🎭 Playing WebM mascot scenario: ${scenario}`);
                
            } catch (error) {
                this.logger.warn('Failed to play mascot animation:', error.message);
    ...
    testAllAnimations() {
        ...
        const playNext = () => {
            if (index < scenarios.length) {
                this.logger.debug(`Testing animation: ${scenarios[index]}`);
                this.playScenario(scenarios[index]);
                index++;
                setTimeout(playNext, 4000); // 4 seconds per animation
            } else {
                this.playScenario('idle');
                this.logger.debug('Animation test complete');
            }
        };
    ...
    checkVideoHealth() {
        ...
        if (!video) {
            this.logger.warn('🎭 Video element not found');
            return false;
        }
        
        this.logger.debug('🎭 Video Health Check:');
        this.logger.debug(`  - Current source: ${video.src}`);
        this.logger.debug(`  - Ready state: ${video.readyState}`);
        this.logger.debug(`  - Network state: ${video.networkState}`);
        this.logger.debug(`  - Can play: ${video.readyState >= 2}`);
        this.logger.debug(`  - Duration: ${video.duration || 'unknown'}`);
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        this.logger.debug('🎭 Global WebM mascot animator initialized');
        ...
        this.logger.debug('🎬 Debug commands available:');
        this.logger.debug('  - testMascotAnimations() - Test all animations');
        this.logger.debug('  - checkMascotHealth() - Check video system status');
    } catch (error) {
        this.logger.error('Failed to initialize mascot animator:', error);
    }
});
